...
'''
函数：rrt_star重选父节点及更新p_nearby的边
输入：新节点p_new, p_new附近的节点列表p_nearby，树tree
返回：无返回值，直接改写tree中p_new的父节点
'''
def new_father(p_new, p_nearby, tree):
    ...
    for i in range(len(p_nearby)):
        t_dis = distance(p_new, p_nearby[i][0]) + cost(p_nearby[i][1], tree)  # 计算根节点到点pi+点pi到点p_new的代价
        if t_dis < min_dis:
            min_dis = t_dis
            min_father = p_nearby[i]
            t = i
    p_nearby.pop(t)
    tree.append([p_new[0], p_new[1], min_father[1]])  # 将p_new加入tree，min_father[1]是其最短父节点的行标
    # 2.更新p_nearby的边
    i_p_new = len(tree)-1  #
    cost_p_new = cost(i_p_new, tree)
    for j in p_nearby:
        pre_cost = cost(j[1], tree)
        if cost_p_new + distance(j[0],p_new) < pre_cost:
            tree[j[1]][2] = i_p_new  # 如果起点到p_new+p_new到pj代价小于起点到pj，更新pj父节点为p_new
